...
command = f"ffmpeg -nostdin -i {INPUT_FILE} -qscale:v {FRAME_QUALITY} {TEMP_FOLDER}/frame%06d.jpg -hide_banner"
subprocess.call(command, shell=True)

command = f"ffmpeg -nostdin -i {INPUT_FILE} -ab 160k -ac 2 -ar {SAMPLE_RATE} -vn {TEMP_FOLDER}/audio.wav"

subprocess.call(command, shell=True)

command = f"ffmpeg -nostdin -i {TEMP_FOLDER}/input.mp4 2>&1"
f = open(os.path.join(TEMP_FOLDER, "params.txt"), "w")
...
f = open(os.path.join(TEMP_FOLDER, "params.txt"), 'r+')
pre_params = f.read()
f.close()
...
for chunk in chunks:
    ...
    for outputFrame in range(startOutputFrame, endOutputFrame):
        ...
        elif lastExistingFrame is not None:
...
command = (f"ffmpeg -nostdin -framerate {frameRate} -i {os.path.join(TEMP_FOLDER, 'newFrame%06d.jpg')} "
           f"-i {os.path.join(TEMP_FOLDER, 'audioNew.wav')} -strict -2 {OUTPUT_FILE}")
subprocess.call(command, shell=True)
