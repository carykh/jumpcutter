...
def main():
    ...
    input_defintion = {
        ...
        '-OUT FILE-': (
        '--output_file', 'Output File', '', (40, 1), "the output file. (optional. if not included, it'll just modify the input file name)", [sg.FileSaveAs()]),
        ...
        '-FRAME RATE-': ('--frame_rate', 'Frame Rate', 30, None,
                         "frame rate of the input and output videos. optional... I try to find it out myself, but it doesn't always work.", []),
        '-FRAME QUALITY-': (
        '--frame_quality', 'Frame Quality', 3, None, "quality of frames to be extracted from input video. 1 is highest, 31 is lowest, 3 is the default.", [])
    }
    ...
    layout += [[sg.Text('Constructed Command Line:')],
               ...
               [sg.MLine(size=(80, 10), reroute_stdout=True, reroute_stderr=False, reroute_cprint=True, write_only=True, font='Courier 8', autoscroll=True,
                         key='-ML-')],
               [sg.Button('Start'), sg.Button('Clear All'), sg.Button('PyCharm Me'), sg.Button('Help'), sg.Button('Exit')],
    ...
    while True:
        ...
        if event in (sg.WIN_CLOSED, 'Exit'):  # if window was closed
            ...
            sg.popup_quick_message('Beginning conversion... this will take a long time... your window may appear',
                                   ...
                                   'Command Line Output area once the conversion has completed', line_width=90, keep_on_top=True, background_color='red',
                                   text_color='white', auto_close_duration=8)
